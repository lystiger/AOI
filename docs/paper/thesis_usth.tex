...
\begin{itemize}
  ...
  \texttt{model\_version} (Figure~\ref{fig:ws6_s09}), demonstrating that the
...
{Supply the S04 Grafana panel showing reduced detector confidence on degraded boards.}
{Scenario S04 (low confidence) from real inference: average detector confidence on blurred/noisy copies of defective boards falls below the baseline, a genuine drop produced by the model rather than a scripted value.\label{fig:ws6_s04}}

\thesisfigure
{../../thesis-assets/11_ws6_real/s09_degradation.png}
{Supply the S09 Grafana panel showing FAIL counts per model\_version dropping from baseline to the under-trained checkpoint.}
{Scenario S09 (model degradation) from real inference: detected defects per \texttt{model\_version} collapse when the under-trained checkpoint replaces the baseline on identical boards.\label{fig:ws6_s09}}
