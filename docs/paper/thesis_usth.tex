...
% --- scenario-gallery panel: real capture or placeholder, with a subcaption+label ---
\newcommand{\scenariopanel}[3]{%
  \begin{subfigure}[t]{0.31\textwidth}\centering
    \IfFileExists{#1}{\includegraphics[width=\linewidth]{#1}}{%
      \fbox{\parbox[c][0.8in][c]{\linewidth}{\centering\scriptsize\textbf{#2}}}}%
    \caption{#3}%
  \end{subfigure}}
...
\newcommand{\thesistitle}{Design and Evaluation of an AI Model Logging and
Monitoring System Using the Loki stack}
...
\subsubsection{Monitoring Scope and Signal Selection}\label{subsubsec:signal_selection}
...
measurement procedure in Section~\ref{subsubsec:perf_resources} and Table~\ref{tab:performance_resources}.
...
specific test scenarios are defined in Section~\ref{subsec:experiments} and summarized in
...
\subsubsection{Deployment}\label{subsubsec:deployment}
...
Section~\ref{subsubsec:results_summary}. This closes the loop from test generation to runtime evidence
...
The experiment can be reproduced in four steps. First, the containerized
FastAPI, Promtail, Loki, and Grafana services are started with the fixed
configuration described in Section~\ref{subsubsec:deployment}. Second,
the S01--S09 scenarios are executed sequentially with the inter-scenario pause
defined above. Third, the provisioned AOI dashboard and the LogQL queries in
Table~\ref{tab:logql_queries} are used to inspect the resulting streams. Finally,
the same Loki records are queried over a fixed 120-minute lookback window to
regenerate the aggregate report. Reproduction therefore depends on the stated
service versions, scenario definitions, timing, and queries rather than on
undocumented manual dashboard operations.
...
Figure~\ref{fig:scenario_gallery} collects the Grafana Explore capture for all
nine anomaly scenarios (S01--S09), each executed with real inference. Every
scenario discussion that follows refers to a panel of that figure.

\begin{figure}[p]\centering
\scenariopanel{../../thesis-assets/11_ws6_real/s01_baseline.png}{S01}{S01 baseline control: PASS dominates FAIL; the comparison condition for every anomaly scenario.\label{fig:s01}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s02_high_defect.png}{S02}{S02 fail-rate anomaly: FAIL-dominated stream under a \texttt{PCB-HIGHFAIL-.*} filter.\label{fig:s02}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s03_latency.png}{S03}{S03 latency spike: \texttt{inference\_latency\_ms} climbs toward a $\sim$2000\,ms plateau while real predictions continue.\label{fig:s03}}

\vspace{0.8em}
\scenariopanel{../../thesis-assets/11_ws6_real/s04_confidence.png}{S04}{S04 low confidence: the detector's own average confidence on degraded boards falls below baseline.\label{fig:s04}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s05_flood.png}{S05}{S05 defect concentration: a single dominant real class (\texttt{SPUR}) isolated by \texttt{defect\_type}.\label{fig:s05}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s06_board_failure.png}{S06}{S06 board-level failure: \texttt{BOARD\_FAILURE} isolated from the corrupt \texttt{PCB-DEAD} boards.\label{fig:s06}}

\vspace{0.8em}
\scenariopanel{../../thesis-assets/11_ws6_real/s07_recovery.png}{S07}{S07 recovery: FAIL and PASS series cross back as the line returns to healthy operation.\label{fig:s07}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s08_intermittent.png}{S08}{S08 intermittent fault: three real defective-board bursts within otherwise clean traffic.\label{fig:s08}}\hfill
\scenariopanel{../../thesis-assets/11_ws6_real/s09_degradation.png}{S09}{S09 model regression: detected defects per \texttt{model\_version} collapse under the under-trained checkpoint.\label{fig:s09}}
\caption{Grafana Explore evidence for all nine anomaly scenarios (S01--S09), each executed with real inference. Every panel is a live capture of the LogQL query for that scenario over the same structured log stream; together they show that each anomaly family produces a distinct, queryable signature.\label{fig:scenario_gallery}}
\end{figure}
...
anomaly scenarios are compared (Figure~\ref{fig:s01}).
...
though real inference outputs continue to arrive (Figure~\ref{fig:s03}).
...
corresponding captures are the remaining panels of Figure~\ref{fig:scenario_gallery}. The
...
\subsubsection{Performance and Resource Consumption}\label{subsubsec:perf_resources}
...
Active label cardinality & 120-minute selector window & 1177 active series; distinct \texttt{pcb\_id}: 131, \texttt{component\_id}: 15, \texttt{defect\_type}: 7, \texttt{inspection\_result}: 2, \texttt{model\_version}: 2 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{Label-Design Ablation: \texttt{pcb\_id} as Label versus Parsed Field}\label{subsubsec:label_ablation}
The label schema in Section~\ref{subsubsec:signal_selection} and
Figure~\ref{fig:event_label_mapping} promotes \texttt{pcb\_id} to a Loki label
while keeping numeric fields parsed. To test that decision rather than assert it,
a controlled ablation holds every factor constant except the one under study:
whether \texttt{pcb\_id} is promoted. One Loki instance ingests the same
synthesized corpus through two Promtail arms---\texttt{aoi-label} (\texttt{pcb\_id}
promoted, the deployed design) and \texttt{aoi-parsed} (\texttt{pcb\_id} extracted
but kept as a parsed JSON field)---so the two arms differ only in that single line
of Promtail configuration. The corpus preserves the deployed run's label
\emph{shapes} (15 components, seven defect classes, two model versions) and adds a
fixed five-board \texttt{PCB-DEAD} isolation set, so the board-isolation query
returns an identical 50-entry result set at every scale while the surrounding
corpus grows. The board count $N$ is swept from 131 (the deployed scale) to 4000,
and cardinality and query latency are measured with the same Loki \texttt{series}
and \texttt{query\_range} endpoints used for Table~\ref{tab:performance_resources}.
The sweep is automated, and the two Promtail pipelines differ only in whether
\texttt{pcb\_id} is promoted to a label; all other ingestion and query settings
are identical.

Table~\ref{tab:label_ablation} and Figure~\ref{fig:label_ablation} report the
outcome. At $N=131$ the label arm reproduces the deployed cardinality (1184 active
series against the 1177 measured in Table~\ref{tab:performance_resources}), which
validates the synthetic corpus against the real system. Three findings follow.

\textbf{Cardinality scales with the labelled identifier.} Promoting \texttt{pcb\_id}
makes active series grow linearly at roughly nine series per board (1184, 4575,
9086, 18098, and 36097 series at $N=131,500,1000,2000,4000$), because each board
multiplies the stream count. Keeping \texttt{pcb\_id} parsed holds the series count
flat at about 225 regardless of $N$, bounded by the product of the remaining
low-cardinality labels.

\textbf{The labelled arm breaks the default deployment.} Under Loki's default
per-tenant limit of 5000 streams, the label arm saturates between 500 and 1000
boards; Loki then silently rejects new streams---including, because they arrive
last, the \texttt{PCB-DEAD} isolation boards, so the board-isolation query returns
nothing. The measurements in Table~\ref{tab:label_ablation} therefore required the
per-tenant stream ceiling to be raised above the largest measured cardinality.
A separate run at the default 5000-stream limit confirmed the resulting data
loss, while the parsed arm never approached the limit.

\textbf{The label does not buy query speed at this scale.} For the realistic
board-isolation query---a regex match \texttt{pcb\_id=\textasciitilde"PCB-DEAD-.*"}
returning the same 50 entries---the parsed-field form
(\texttt{| json | pcb\_id=\textasciitilde\ldots}) is as fast or faster than the
label-selector form at every tested $N$ (9.0 versus 16.7\,ms at $N=131$, rising to
20.3 versus 32.8\,ms at $N=4000$). Because a regex label selector forces Loki to
match the pattern against every distinct \texttt{pcb\_id} value in the index, the
labelled path carries more overhead than a linear scan over a corpus this small;
the scan cost that would eventually favour the label only dominates at data volumes
well beyond a 4000-board run.

Taken together, these results reframe the labeling decision empirically. The
18.8\,ms board-isolation latency in Table~\ref{tab:performance_resources} is not
attributable to the label---the parsed equivalent is comparable---so promoting
\texttt{pcb\_id} is justified by dashboard label-navigation and forward
compatibility rather than current query performance, and its measured cost is a
cardinality that becomes operationally prohibitive under default limits by roughly
1000 boards. This confirms, with measurement, the scale-bounded reading given in
Section~\ref{subsubsec:signal_selection}: at the 131-board prototype the cost is
bounded and the choice is defensible, whereas at production scale \texttt{pcb\_id}
should move to a parsed field or to structured metadata. The decision is thus
demonstrated rather than asserted.

\begin{table}[!ht]
\centering
\caption{Label-design ablation: active Loki series and board-isolation query latency for \texttt{pcb\_id} promoted to a label versus kept as a parsed field, over a board-count sweep. Both arms ingest an identical corpus; the board query returns the same 50-entry result set at every $N$.}
\label{tab:label_ablation}
\begin{tabular}{rrrrr}
\toprule
& \multicolumn{2}{c}{\textbf{Active Loki series}} & \multicolumn{2}{c}{\textbf{Board query (ms, median)}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
\textbf{Boards $N$} & label & parsed & label selector & parsed filter \\
\midrule
131  & 1184  & 190 & 16.7 & 9.0 \\
500  & 4575  & 222 & 19.1 & 10.6 \\
1000 & 9086  & 224 & 27.1 & 12.4 \\
2000 & 18098 & 225 & 24.2 & 15.7 \\
4000 & 36097 & 225 & 32.8 & 20.3 \\
\bottomrule
\end{tabular}
\end{table}

\thesisfigure
{../../thesis-assets/12_label_ablation/label_ablation.png}
{Label-design ablation: cardinality and board-query latency versus board count.}
{Label-design ablation over a board-count sweep. (a) Active Loki series (log scale): promoting \texttt{pcb\_id} to a label grows series linearly and crosses Loki's default 5000-stream limit between 500 and 1000 boards, while the parsed-field arm stays flat near 225. (b) Median latency of the board-isolation query for an identical 50-entry result set: the parsed-field filter is as fast or faster than the regex label selector at every tested scale.\label{fig:label_ablation}}

\FloatBarrier

\subsubsection{Measured Results Summary}\label{subsubsec:results_summary}
...
no defect (106 events), followed by mouse bite (41), spur (35), conductor
scratch (33), base-material foreign object (31), hole breakout (28), open
circuit (27), spurious copper (22), conductor foreign object (17), board
failure (8), and short (2). Each passing inspection is categorized as no
defect, so the two counts coincide.
...
Alerts fired during execution & S02, S03 thresholds crossed \\
\bottomrule
\end{tabular}
\end{table}
...
arrive, and recovery scenarios produced clear state transitions over time; the
consolidated view of these signals is the anomaly dashboard in
Figure~\ref{fig:ws6_dashboard}.
...
defined in Section~\ref{subsec:experiments} and summarized in Table~\ref{tab:scenario_suite}; this
...
Finally, the label schema is bounded to the prototype scale. The ablation in
Section~\ref{subsubsec:label_ablation} quantifies this: promoting \texttt{pcb\_id}
to a Loki label grows active series linearly with the board count and exceeds
Loki's default 5000-stream limit at roughly 1000 boards, at which point the
default deployment silently drops data. The prototype's 131 boards sit well
inside this bound, so the choice is defensible here, but a production-scale
deployment would need to demote \texttt{pcb\_id} to a parsed field or to
structured metadata.
...
Future work can extend the platform in five directions:
\begin{itemize}
    ...
    \item Calibrate the existing alert thresholds against representative production data;
    \item Demote high-cardinality identifiers such as \texttt{pcb\_id} from Loki labels to parsed fields or structured metadata before production scale, as quantified by the label-design ablation (Section~\ref{subsubsec:label_ablation});
...
\bibitem{hctaoi} HCT, ``SMT Line Automatic AOI Testing Equipment,'' product documentation, available at: \href{https://www.hct-smt.com/products/aoi-testing-equipment/smt-line-automatic-aoi-testing-equipment}{\nolinkurl{https://www.hct-smt.com/}}, accessed June 29, 2026.
