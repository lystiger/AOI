...
%% TODO(human): Confirm with FUYU that the Holly training-deck image may be reproduced in the public thesis.
\begin{figure}[h]
\centering
\includegraphics[width=0.38\columnwidth]{../../thesis-assets/01_aoi_machine/holly.png}
\caption{Holly AOI machine and operator station. Source: \textit{Holly AOI Training Guideline}, Shanghai Holly Electronics Co., Ltd. Reproduced subject to host-organization permission.}
...
\vspace{0.3em}
\raggedright\footnotesize Source taxonomy: the repository's
\texttt{docs/defects.md}, organized against IPC-A-610H~\cite{ipc610h}.
\end{table}
...
measurement procedure in Section~4.1.7. Grafana was retained because the
...
runs, and event ingestion. The frontend is a React workstation for PCB review,
...
within a single system. It also includes synthetic event generation, which is
useful for testing failure scenarios without requiring a fully deployed
production model. This makes the platform suitable for controlled
experimentation and report demonstration.
...
% \end{minipage}}
% \caption{Runtime lifecycle of one synthetic experiment event. The event passes through the production-shaped API, validation, persistence, shipping, storage, visualization, and reporting stages; the resulting evidence therefore validates the observability pipeline rather than detector accuracy.}
...
{Runtime lifecycle of one synthetic experiment event. The event passes through the production-shaped API, validation, persistence, shipping, storage, visualization, and reporting stages; the resulting evidence therefore validates the observability pipeline rather than detector accuracy.\label{fig:experiment_workflow}}
...
These tests therefore evaluate observability behavior under controlled
synthetic inputs. They do not estimate precision, recall, or other detector
accuracy measures, and they do not establish production readiness of a future
real AI model.

\subsubsection{Experiment Harness: Build, Run, and Report}
The repository separates the experiment lifecycle into three stages that a judge
...
window, and writes a Markdown report to
\texttt{docs/experiments/anomaly\_results.md}. This closes the loop from test
generation to runtime evidence to report artifact.
...
This three-part split is a deliberate design choice. It makes the experiment
suite reproducible, easier to extend, and easier to defend academically because
the judge can inspect how scenarios are generated, how they are injected, and
how final evidence is computed.
...
Synthetic scenarios & Production-line data & Controlled inputs and repeatable anomaly classes without claiming detector accuracy & \cite{paleyes2022challenges} \\
\bottomrule
\end{tabular}
\end{table}
...
{Scenario S09 trend evidence in Grafana Explore. The query unwraps \texttt{confidence\_score}, and the graph preserves the injected decline from approximately 0.95 to 0.45; this validates observability of a synthetic drift-like pattern, not detector accuracy.}
...
freshness comes from \path{docs/experiments/freshness.json}; the container
snapshot comes from \path{docs/experiments/docker_stats.json}; and storage
growth comes from \path{docs/experiments/storage_growth.json}. Query timing and
cardinality are taken from \path{docs/experiments/logql_latency.json} and
\path{docs/experiments/label_cardinality.json}, respectively. Means are the
JSON-reported means, and the displayed sample standard deviations are computed
from the recorded sample arrays in the same files.
...
The retained reports \path{docs/experiments/run_1.md},
\path{docs/experiments/run_2.md}, and \path{docs/experiments/run_3.md} each
contain 913 Loki events. Because those reports do not record the corresponding
submitted-event count, they demonstrate repeatable observed ingestion volume
but do not support a per-run completeness percentage. The ten freshness trials
provide the available variance estimate of $1258.0 \pm 75.8$\,ms.

\thesisfigure
{../../thesis-assets/09_performance/container-resources.png}
{Measured CPU and memory chart from the per-container snapshot.}
{CPU and memory consumption of the running Compose services from \texttt{docker\_stats.json}.\label{fig:container_resources}}

\thesisfigure
{../../thesis-assets/09_performance/storage-growth.png}
{Measured JSONL and Loki storage-growth chart.}
{JSONL and Loki storage growth for 1000 accepted events from \texttt{storage\_growth.json}.\label{fig:storage_growth}}
...
\begin{table}[h]
\centering
\caption{Suite-Level Results From \texttt{anomaly\_results.md}}
...
$1258.0 \pm 75.8$\,ms across ten trials in
\path{docs/experiments/freshness.json}. This is the empirical lower bound for
...
In addition, the experiments were conducted using a local AOI workstation and synthetic event generation. The system was not evaluated on a fully deployed production line with real-time image acquisition, operator variability, equipment state changes, or delayed human feedback. In a practical manufacturing environment, additional factors such as machine status, production throughput, board-mix variation, and review latency would also need to be monitored.
...
and gradual degradation observable through the same stack. Because these
conditions were injected as synthetic events with known ground truth, the work
validates the observability pipeline---its ability to surface and isolate such
patterns---rather than the predictive accuracy of the AOI detector itself.
