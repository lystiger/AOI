...
The harness is version-controlled (\texttt{experiments/ablation\_sweep.py} with the
two \texttt{deploy/promtail/config.ablation-*.yml} arms).
...
stream limit to be lifted (\texttt{deploy/loki/config.ablation.yml}); the
default-limit run that exhibits the data loss is retained as
\texttt{docs/experiments/label\_ablation\_defaultlimit.json}. The parsed arm never
...
\begin{itemize}
    ...
    \item Demote high-cardinality identifiers such as \texttt{pcb\_id} from Loki labels to parsed fields or structured metadata before production scale, as quantified by the label-design ablation (Section~\ref{subsubsec:label_ablation}); and
